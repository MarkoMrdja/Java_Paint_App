...
public class ModDonut extends JDialog implements ActionListener {
	...
	public ModDonut() {
		...
		}
		{
			...
			gbc_txtInner.anchor = GridBagConstraints.WEST;
			gbc_txtInner.insets = new Insets(0, 0, 5, 5);
			gbc_txtInner.gridx = 1;
			gbc_txtInner.gridy = 3;
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if(!Utilities.isNumeric(txtX.getText()) || !Utilities.isNumeric(txtY.getText()) || !Utilities.isNumeric(txtRadius.getText()) || !Utilities.isNumeric(txtInner.getText())) {
							JOptionPane.showMessageDialog(null, "Letters and negative number are not allowed as a input!");
						}
						else if(Integer.parseInt(txtX.getText()) <= 0 || Integer.parseInt(txtY.getText()) <= 0 || Integer.parseInt(txtRadius.getText()) <= 0 || Integer.parseInt(txtInner.getText()) <= 0) {
							JOptionPane.showMessageDialog(null, "Letters and negative number are not allowed as a input!");
						}
						else {
							isOk = true;
